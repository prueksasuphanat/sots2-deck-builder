...
function onDescMouseLeave() { showTooltip.value = false; }
</script>
...
<template>
  ...
  <div
    ...
    @mouseenter="(e) => { if (!selected) (e.currentTarget as HTMLElement).style.borderColor = 'var(--border-strong)'; }"
    @mouseleave="(e) => { if (!selected) (e.currentTarget as HTMLElement).style.borderColor = 'var(--border)'; }"
  >
    <span
    ...
    <div class="w-8 h-8 flex-shrink-0 rounded overflow-hidden flex items-center justify-center" style="background: var(--bg-elevated)">
      <img v-if="imageUrl && !imgError" :src="imageUrl" :alt="card.name" class="w-full h-full object-cover" @error="onImgError" />
      <span v-else class="text-base">{{ tc.icon }}</span>
    </div>

    <div class="flex-1 min-w-0">
      <p class="text-sm font-medium leading-tight truncate" style="color: var(--text-primary)">{{ card.name }}</p>
    ...
    <div v-if="count !== undefined && !inDeck" class="flex items-center gap-0.5 flex-shrink-0">
      <button class="w-5 h-5 rounded text-xs flex items-center justify-center transition-colors hover:text-red-400"
              style="border: 1px solid var(--border-strong); color: var(--text-muted)" @click.stop="$emit('remove', card)">−</button>
      <span class="w-5 text-center text-xs font-mono" style="color: var(--accent)">{{ count }}</span>
      <button class="w-5 h-5 rounded text-xs flex items-center justify-center transition-colors hover:text-emerald-400"
              style="border: 1px solid var(--border-strong); color: var(--text-muted)" @click.stop="$emit('add', card)">+</button>
    </div>
  ...
  >
    ...
    <div class="flex items-center gap-1.5 px-2.5 pt-2.5 pb-1.5">
      <span class="w-7 h-7 flex-shrink-0 flex items-center justify-center text-xs font-display font-bold rounded border"
            :class="[cc.border, cc.accent]" style="background: var(--bg-base)">{{ costLabel }}</span>
      <p class="text-xs font-semibold leading-tight flex-1 min-w-0 truncate" style="color: var(--text-primary)">{{ card.name }}</p>
    </div>

    <div class="mx-2.5 rounded overflow-hidden flex items-center justify-center relative"
         style="height: 96px; background: var(--bg-elevated); border: 1px solid var(--border)">
      <div v-if="imageUrl && !imgLoaded && !imgError" class="absolute inset-0 skeleton" />
      <img v-if="imageUrl && !imgError" :src="imageUrl" :alt="card.name"
    ...
    <div class="px-2.5 pt-1.5 pb-1 flex-1">
      <p class="card-desc text-xs leading-relaxed" style="color: var(--text-secondary)"
         v-html="descHtml(card.description)"
         @mouseenter="onDescMouseEnter" @mouseleave="onDescMouseLeave" />
    </div>

    <div v-if="card.keywords?.length" class="flex flex-wrap gap-1 px-2.5 pb-1.5">
      <span v-for="kw in card.keywords" :key="kw"
            class="text-xs px-1.5 py-0.5 rounded-full font-medium"
            style="background: var(--accent-dim); color: var(--accent); border: 1px solid var(--accent)">{{ kw }}</span>
    </div>

    <span class="absolute top-2 right-2 text-xs font-medium" :class="cc.accent" style="z-index:10">
    ...
    <div v-if="selectable"
      ...
      <button class="btn-accent text-xs px-4 py-2" @click.stop="$emit('add', card)">+ Add to Deck</button>
    </div>

    <div v-if="count && count > 0" class="text-center font-bold relative z-20"
         style="background: var(--accent); color: var(--bg-base)">{{ count }}</div>
  </div>

  <!-- Tooltip -->
  <Teleport to="body">
    <transition name="tooltip-fade">
      <div v-if="showTooltip" class="tooltip-popup" :style="{ left: tooltipX + 'px', top: tooltipY + 'px' }">
...
.tooltip-popup {
  ...
  max-width: 280px; min-width: 200px; transform: translate(-50%, -100%);
}
.tooltip-content {
...
.tooltip-arrow {
  ...
  border-bottom: 1px solid var(--border-strong); transform: translateX(-50%) rotate(45deg);
}
