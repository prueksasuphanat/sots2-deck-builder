...
const totalPages = computed(() => Math.ceil(cardsStore.sortedCards.length / itemsPerPage));
...
const paginationInfo = computed(() => {
  ...
  const end = Math.min(currentPage.value * itemsPerPage, cardsStore.sortedCards.length);
...
watch(() => cardsStore.sortedCards.length, () => {
...
function onCardDragStart(e: DragEvent, card: ApiCard) {
  ...
  if (cardEl) {
    ...
    clone.style.opacity = "1"; clone.style.transform = "rotate(-5deg) scale(1.15)";
    clone.style.pointerEvents = "none"; clone.style.zIndex = "9999";
    clone.style.filter = "drop-shadow(0 15px 35px rgba(0,0,0,0.6))";
...
function onCardDragEnd() { isDragging.value = false; draggedCard.value = null; }
</script>
  ...
  <div class="space-y-3">
    <div class="flex items-center gap-3 flex-wrap">
      <div class="flex-1 min-w-0">
        <h1 class="font-display text-xl font-semibold" style="color: var(--text-primary)">Deck Builder</h1>
        <p class="text-sm" style="color: var(--text-muted)">
          <template v-if="cardsStore.loading"><span class="animate-pulse-soft">Loading cards…</span></template>
          <template v-else>
            <span style="color:var(--accent)">{{ cardsStore.sortedCards.length }}</span> cards available
            <span v-if="!decksStore.activeDeckId" class="ml-2 text-red-400 text-xs">← create or select a deck first</span>
          </template>
        </p>
      </div>
      <div class="flex gap-2">
        <button class="btn text-sm" @click="showFilters = !showFilters">🔧 {{ showFilters ? "Hide" : "Filters" }}</button>
        <button class="btn-accent text-sm lg:hidden relative" @click="showDeckMobile = !showDeckMobile">
          🗂 Deck
          <span v-if="decksStore.activeDeck" class="ml-1.5 text-xs rounded-full px-1.5 py-0.5 font-bold"
                style="background:var(--accent);color:var(--bg-base)">{{ decksStore.totalCards(decksStore.activeDeck) }}</span>
        </button>
      </div>
    </div>
    ...
    <div class="relative">
      ...
      <input :value="cardsStore.filters.search" type="text" placeholder="Search cards…" class="input pl-9 text-sm"
    ...
    <div class="flex gap-4 lg:gap-6">
      <div class="flex-1 min-w-0">
        ...
        <div v-else-if="cardsStore.error" class="flex flex-col items-center py-16 text-center">
          <p class="text-4xl mb-3">⚠️</p>
          <p class="font-medium mb-1" style="color:var(--text-primary)">Failed to load cards</p>
          <p class="text-sm mb-4" style="color:var(--text-muted)">{{ cardsStore.error }}</p>
          <button class="btn-accent" @click="cardsStore.load()">Try Again</button>
        </div>
        <div v-else-if="!cardsStore.sortedCards.length" class="flex flex-col items-center py-16 text-center">
          <p class="text-4xl mb-3">🔮</p>
          <p class="font-medium mb-1" style="color:var(--text-secondary)">No cards match</p>
          <button class="btn mt-3" @click="cardsStore.resetFilters()">Reset Filters</button>
        </div>
        <div v-else-if="cardsStore.viewMode === 'grid'" class="flex flex-wrap gap-3">
          <div v-for="card in paginatedCards" :key="card.id"
               class="drag-card-wrapper transition-all duration-150"
               :class="{ 'opacity-50 scale-95': isDragging && draggedCard?.id === card.id }"
               :draggable="!!decksStore.activeDeckId"
               @dragstart="(e) => onCardDragStart(e, card)"
               @dragend="onCardDragEnd">
            <CardItem :card="card" :count="getCount(card)" :selectable="!!decksStore.activeDeckId"
                      @add="addCard" @click="addCard" />
          </div>
        </div>
        <div v-else class="space-y-1.5">
          <div v-for="card in paginatedCards" :key="card.id"
               ...
               @dragstart="(e) => onCardDragStart(e, card)"
               @dragend="onCardDragEnd">
            <CardItem :card="card" :count="getCount(card)" compact @add="addCard" @click="addCard" />
          </div>
        </div>
      ...
      <aside class="hidden lg:flex flex-col flex-shrink-0 w-72 xl:w-80" style="min-height:calc(100vh - 120px)">
        <DeckPanel />
      </aside>
    </div>

    <transition name="slide-up-sheet">
      <div v-if="showDeckMobile"
        ...
        <div class="flex items-center justify-between mb-3">
          <p class="font-semibold" style="color:var(--text-primary)">Your Deck</p>
          <button class="btn !px-2 !py-1 text-xs" @click="showDeckMobile = false">✕ Close</button>
        </div>
        <div class="flex-1 overflow-y-auto"><DeckPanel /></div>
      </div>
    </transition>
    <div v-if="showDeckMobile" class="lg:hidden fixed inset-0 z-30" style="background:rgba(0,0,0,0.5)" @click="showDeckMobile = false" />
  </div>
</template>
...
.drag-card-wrapper { cursor: grab; }
.drag-card-wrapper:active { cursor: grabbing; }
</style>
