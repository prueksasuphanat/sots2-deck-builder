...
const totalPages = computed(() => {
  return Math.ceil(store.sortedCards.length / itemsPerPage);
});
...
// Upgrade info as readable list
const upgradeItems = computed(() => {
  if (!selected.value?.upgrade) return [];
  return Object.entries(selected.value.upgrade).map(([k, v]) => ({
    stat: k,
    change: v,
  }));
});
  ...
  <div class="space-y-4">
    ...
    <div class="flex gap-4 lg:gap-6">
      ...
      <aside
        ...
        :class="
          showFiltersMobile
            ? 'block w-full sm:w-72'
            : 'hidden sm:block sm:w-64 lg:w-72'
        "
      >
      ...
      <div class="flex-1 min-w-0">
        ...
        <!-- Empty state -->
        <div
          v-else-if="!store.sortedCards.length"
          class="flex flex-col items-center py-20 text-center"
        >
          ...
          <button class="btn mt-3" @click="store.resetFilters()">
            Reset Filters
          </button>
        </div>
      ...
      <transition name="detail">
        <aside
          v-if="selected"
          class="hidden lg:block flex-shrink-0 w-64 xl:w-72"
        >
          <div
            class="sticky top-20 rounded-lg p-4 space-y-4"
            style="
          ...
          >
            ...
            <!-- Stats block -->
            <div
              class="rounded p-3 text-xs space-y-1.5"
              style="
            ...
            >
              <div class="flex justify-between" v-if="selected.damage">
                ...
                <span class="font-mono text-red-400"
                  >{{ selected.damage }}
                  <span v-if="selected.hit_count && selected.hit_count > 1">
                    × {{ selected.hit_count }}</span
                  >
                </span>
              </div>
            ...
            <div v-if="selected.powers_applied?.length">
              ...
              <div class="space-y-1">
                <div
                  v-for="p in selected.powers_applied"
                  :key="p.power"
    ...
    <transition name="slide-up-sheet">
      ...
      >
        <div class="flex items-start justify-between mb-3 gap-3">
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs mt-0.5" style="color: var(--text-muted)">
              {{ selected.color }} · {{ selected.type }} · {{ selected.rarity }}
            </p>
          </div>
          <button
            class="btn !px-2 !py-1 text-xs flex-shrink-0"
            @click="selected = null"
          >
            ✕
          </button>
        ...
        <p
          class="card-desc text-sm leading-relaxed"
          style="color: var(--text-secondary)"
          v-html="descHtml(selected.description)"
        />
        <div
          v-if="upgradeItems.length"
